...
## 2026-08-26 - Phase 11: 제품 목표 기준의 화면 검토

3층 기준으로 화면을 하나씩 봤다. 질문은 하나다. **이 화면이 "달릴지 말지"의 판단을 진전시키는가.**

### 요소 상세: 정보가 없는 자리가 가장 컸다

사진이 없을 때 자리 표시 상자가 시트의 절반을 차지하고, 정작 판단 재료인 설명은 그 아래에서 잘렸다. 사진이 있으면 맞는 배분이지만 없을 때는 정확히 뒤집힌다.

사진이 없으면 상자를 그리지 않고 설명을 앞세우도록 했다. 설명의 색도 보조에서 기본으로 올렸다. 사진이 있을 때의 배치는 그대로다.

### 목록 카드: 고를 재료가 없었다

CoCo가 다른 앱과 다른 지점은 요소인데, 요소 개수는 코스를 선택해야만 보였다. 후보를 좁히는 단계에서는 작성자가 고른 난이도 3단계가 유일한 단서였다.

접힌 카드에도 요소 요약을 넣었다. 0개인 분류는 표시하지 않는다. 모든 카드에 0을 늘어놓으면 높이만 늘고 고르는 데는 도움이 되지 않는다. 순서는 확장 카드와 같게 맞췄다.

### 반응과 난이도의 비중: 내 판단이 틀렸다

"실제로 달려본 사람들의 반응이 작성자 주관인 난이도보다 작게 표시된다"고 적었으나, 화면을 다시 보니 사실이 아니었다. 난이도는 이름 옆의 작은 회색 글자고 반응은 44pt 캡슐 버튼이다. 오히려 반응 쪽이 무겁다. 고칠 것이 없어 넘어갔다.

화면을 열어보지 않고 기억으로 판단한 결과였다.

### 남긴 것

데이터나 입력 구조가 필요한 것은 `BACKLOG.md`로 옮겼다. 등록 시 구조화된 정보, 마커 겹침 처리, 고도 정보, 사진 유도 네 가지다.

특히 마지막은 정직하게 적어둘 필요가 있다. 요소 상세에서 빈 사진 자리를 없앤 것은 사진이 없다는 사실을 덜 거슬리게 만든 것이지, 사진이 없는 문제를 푼 것이 아니다.

## 2026-08-26 - Phase 11: 아이콘과 색 체계 정리

색이 그때그때 정해져 같은 색이 두 가지를 뜻하고 있었다. 주황은 주의이면서 자유 그리기 경로, 파랑은 편의이면서 GPX 경로였다. HIG color 문서의 첫 규칙("같은 색을 다른 의미로 쓰지 마라")을 정면으로 어기고 있었다.

의미 축으로 다시 배정했다. 근거는 `DECISIONS.md` D30.

| 대상 | 이전 | 이후 |
|---|---|---|
| 경관 | 카메라 · 보라 | 쌍안경 · teal |
| 주의 | 삼각 경고 · 주황 | 그대로 (주황을 이 의미 전용으로) |
| 편의 | 구급 가방 · 파랑 | 물방울 · indigo |
| 도착 | 체크 깃발 · 빨강 | 체크 깃발 · 짙은 회색 |
| 경로 출처 배지 | 주황/파랑 | 중립 |

카메라는 "사진 찍는 행위"라 경관 자체가 아니었고, 구급 가방은 응급처치로 읽혀 화장실·음수대·휴식을 담는 편의와 어긋났다. 도착의 빨강은 주의의 주황과 인접해 경고를 묻히게 했다.

결과적으로 지도에서 주황이 유일하게 튀는 색이 되어 "조심할 곳"이 바로 보인다.

새 색은 기존 `ColorContrastTests`로 기준 통과를 확인했다. 라이트·다크, Increase Contrast 켜짐·꺼짐 모두 통과한다.
...
## 2026-08-28 - 소셜 로그인 필수화 (feature/required-social-login)

게스트 우선 구조를 접었다. 판단 근거와 대안은 `DECISIONS.md` D31에 정리했다.

계기는 "기기를 바꾸면 못 찾는 게 맞는 구조냐"는 물음이었다. 확인해 보니 코스와 스크랩은 서버 DB에 user_id로 묶여 멀쩡히 있는데, 게스트는 신원을 증명할 수단이 기기의 토큰 하나뿐이라 새 기기에서 그 계정에 닿을 방법이 없었다. 데이터가 지워지는 것이 아니라 주인을 잃는 구조였다.

### 서버

- **토큰 슬라이딩 만료**: 인증에 성공할 때마다 만료를 30일 뒤로 민다. 이전에는 30일간 앱을 안 열면 만료됐고, 앱은 그 토큰을 버리고 새 게스트를 발급해 사용자가 조용히 강등됐다. 갱신 폭이 12시간 미만이면 쓰기를 건너뛴다.
- **게스트 발급 기본 차단**: `coco.auth.guest-issuance-enabled` 기본값이 꺼짐이다. 운영에서 누구나 계정을 찍어낼 수 있는 엔드포인트를 열어둘 이유가 없다. 테스트는 OAuth 왕복 없이 인증해야 하므로 필요한 클래스에서만 켠다.
- **V6 마이그레이션**: 이미 발급된 게스트 토큰을 폐기한다. 남겨두면 그 기기에서는 계속 통과되어 "모든 계정은 소셜 제공자에서 온다"는 전제가 깨진다. 계정과 코스는 지우지 않고 접근만 끊는다.

테스트 설정을 `src/test/resources/application.yml`로 두려다 되돌렸다. 그 파일은 본 설정에 얹히는 것이 아니라 통째로 **대체**해서 소셜 설정까지 사라졌다. 필요한 테스트 클래스에 `@SpringBootTest(properties = ...)`로 주는 방식으로 바꿨다.

### iOS

- `SessionStore`가 토큰 보유 여부를 들고 있고, 앱 최상단에서 로그인 화면과 본 화면을 가른다.
- 401을 받으면 게스트를 발급받지 않고 로그아웃 처리한 뒤 로그인 화면으로 보낸다. 이전 계정에 그 사람의 코스가 있고, 그곳에 닿을 수 있는 것은 소셜 신원뿐이다.
- `issueGuestToken`과 게스트 응답 타입을 지웠다.
- 프로필의 계정 절은 로그인 수단을 안내하는 대신 연결된 제공자를 보여준다. 이 화면에 오는 사람은 이미 로그인한 상태다.

### 검증

서버 42개, iOS 78개 통과. 시뮬레이터에서 게스트 토큰이 폐기된 상태로 앱을 다시 띄워 로그인 화면으로 돌아가는 것을 확인했다.

실제 소셜 로그인 왕복은 아직 확인하지 못했다. 로컬 서버에 Naver·Kakao 자격 증명이 없어서다. 운영 API를 상대로 하거나 로컬에 자격 증명을 넣어야 한다.
