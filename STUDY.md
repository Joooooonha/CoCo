...
## 2026-07-22 - Phase 1.1

### Learning keywords

- Swift value types and `struct`
...
## 2026-08-17 - Phase 10a

### 학습 키워드

- 사전 서명(presigned) URL이 저장소 왕복 없이 로컬 서명 연산으로 발급된다는 점
- SigV4 `X-Amz-SignedHeaders`로 `content-type`·`content-length`까지 서명해 업로드 내용을 고정하기
- `URLCache`가 쿼리를 포함한 URL 전체를 키로 쓰기 때문에 서명 URL과 함께 쓸 수 없는 이유
- 캐시 키를 주소가 아니라 내용의 버전(업로드 시각)으로 잡는 방식
