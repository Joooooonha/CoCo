...
| F12 | 지도 영역 필터 | 코스 목록과 개수는 현재 보이는 지도 영역과 겹치는 코스만 보여준다. 선택된 코스는 항상 포함한다. |
| F13 | 코스 검색 | 시트의 검색창으로 이름·설명·지역·작성자를 텍스트 필터링한다. 서버 검색과 지오코딩은 사용하지 않는다. |
...
| F11 | 표시 이름 변경 | 보관함에서 코스에 표시되는 게스트 이름을 1~20자로 바꿀 수 있다. |
| F14 | 코스 삭제 | 작성자는 보관함의 내 코스에서 스와이프로 코스를 삭제한다. 연관 요소·스크랩·반응이 함께 삭제된다. |
...
지도 탭 계획 대신, 다른 서비스에서 만든 GPX 파일로 경로를 가져올 수 있다.

1. 등록 탭에서 GPX 가져오기를 선택하고 파일 앱에서 GPX 파일을 고른다.
2. 앱이 `trkpt` 좌표 시퀀스를 경로로 읽고, 확장 메타데이터의 거리·시간이 있으면 사용한다. 없으면 좌표로 거리를 계산하고 보행 속도로 시간을 추정한다.
3. 가져온 경로를 지도에서 미리 확인한 뒤 기존 등록 2단계(정보 입력, 요소 추가)를 그대로 진행한다.
4. 이 코스의 `routeSource`는 `IMPORTED_GPX`로 저장한다.
...
최상위 배치는 사용자 결정에 따라 iOS 탭 바로 확정했다. 탭은 탐색, 보관함과 세 번째 등록 탭이며, 코스 등록은 등록 탭에서 시작한다. 스크랩과 반응 버튼은 선택된 코스 요약 카드에 배치한다. 새로운 화면을 임의로 완성하지 않고, 첫 구현 전에 와이어 수준의 상태와 이동을 사용자에게 확인받는다.
...
- 사용자 결정으로 시트는 최소·최대 높이 사이에서 자유롭게 드래그해 원하는 높이에 멈출 수 있다(free-stop). 그래버·헤더 영역 드래그로 높이를 조절한다.
- 시트를 아래로 밀어 완전히 닫을 수 없다. 최소 높이에서도 헤더 요약이 보인다.
- 어떤 높이에서도 뒤쪽 지도와 요소를 조작할 수 있어야 한다.
- 시트 안 코스 목록은 시트 높이와 무관하게 항상 스크롤할 수 있다. 접기·펼치기 버튼은 최소·최대 높이로 이동하는 보조 수단이다.
- 모달 `.sheet`는 탭 바를 가려 최상위 탭 이동을 막으므로 탐색 탭 내부의 커스텀 바텀 패널로 구현한다.
- 시트를 올려도 뒤 지도는 리사이즈되지 않고 시트가 지도를 덮는다. 지도와 시트 높이는 서로 독립이다.
- 요소 상세는 별도 오버레이가 아니라 하프시트 콘텐츠로 표시한다. 뒤로 가기로 코스 목록에 돌아간다. 화면에 겹치는 표면은 지도, 하프시트, 탭 바 세 가지를 넘지 않는다.
...
| `DELETE` | `/api/v1/courses/{courseId}` | 작성자의 코스 삭제 |
...
- `POST /api/v1/courses` 요청은 이름·요약·난이도·거리·시간·`routeSource`·경로 지점 2개 이상·요소 1개 이상을 필수로 한다.
- 소유자는 항상 서버가 토큰에서 결정하며 요청의 소유자 값은 받지 않는다.
- MVP에서 `routeSource`는 `PLANNED_MAPKIT`과 `IMPORTED_GPX`만 허용하고 다른 값은 `400 ROUTE_SOURCE_UNSUPPORTED`로 거부한다.
- 경로 지점 `sequence`는 0부터 시작하는 연속 값이어야 하며 위반 시 `400 ROUTE_POINTS_INVALID`를 반환한다.
- `locationLabel`은 선택 값이며 생략 시 `서울`로 저장한다. 지오코딩은 MVP 범위가 아니다.
- 자원 보호를 위해 경로 지점 2,000개, 요소 50개를 상한으로 둔다.
...
- 지오코딩과 주소·장소 검색 (코스 텍스트 검색과 지도 영역 필터는 F12·F13으로 MVP에 포함)
- 현재 위치와 위치 권한
